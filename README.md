...
- [ ] Falco: Detect suspicious shell access, container escape attempts 
- [ ] OWA: Validates compliance rules (e.g., duplication < 2%, bugs < 1%).
...
```
Welcome to **Helm**! This is a massive step forward. Helm allows you to stop hard-coding values like `1001` or `my-ns` into your YAML files and instead makes them **dynamic variables**.

In Helm, the `values.yaml` acts as your "control panel." You define the values there, and then your `deployment.yaml` template "plucks" those values out.

---

### 1st Way Is Map the values in `deployment.yaml` 
Now, go to your Helm template (usually in `templates/deployment.yaml`). You need to use the Go-templating syntax `{{ .Values... }}` to pull those numbers in.
    ...
    spec:
      containers:
        - name: {{ .Chart.Name }}
          ...
          # --------------------------------
```
### 2nd way is Using `toYaml` 
Most DevOps engineers don't map every single line manually. Instead, they use a Helm helper called `toYaml`. This is much cleaner because if you add a new security setting to `values.yaml`, it automatically shows up in the Deployment without changing the template.

**Try this version in your **`**deployment.yaml**`**:**
...
- name: {{ .Chart.Name }}
  ...
  {{- end }}
```
- `**with**` : Tells Helm to look inside the `securityContext`  section of `values.yaml` .
- `**toYaml .**` : Copies everything from that section exactly as it is written.
- `**nindent 12**` : Automatically adds 12 spaces of indentation so the YAML stays valid.
###  What is `nindent 12`?
In YAML, **indentation is everything**. If a line is shifted even one space too far to the left or right, Kubernetes will reject the file.

When Helm pulls data from your `values.yaml`, it doesn't know where that data is going to land in your `deployment.yaml`.

- The `n`  in `nindent`  stands for **New line**.
- The `12`  means **"Move this text 12 spaces to the right."**
In a Deployment, the `securityContext` inside a container is usually nested 12 spaces deep (2 for `spec`, 2 for `template`, 2 for `spec`, 2 for `containers`, 2 for `- name`, 2 for `securityContext`). If you don't use `nindent 12`, the values will be pasted at the very beginning of the line, and your YAML will break.
...
want only one, two outoff many (two option ignore which you dont want or add which you want)

#### Scenario A: You want EVERYTHING in the block
If you want to copy the entire `securityContext` block from `values.yaml` exactly as it is, use the `with` + `toYaml` combo. It is a "bulk copy-paste."
...
containers:
- name: my-app
  {{- with .Values.containerSecurity }}
  securityContext:
    {{- toYaml . | nindent 12 }}
  {{- end }}
```
#### Scenario B.1: You want ONLY ONE specific value (The "Targeted" approach)
If you only want one specific value (like `runAsNonRoot`) because you want to use the other values somewhere else (like in a different container or a pod-level setting), **do not use **`**toYaml**`. Instead, call the variable directly.
...
runAsUser: 1001
allowPrivilegeEscalation: false
readOnlyRootFilesystem: true  # <--- Suppose you DON'T want this one
```
**Deployment.yaml:**
...
- name: my-app
  securityContext:
    runAsNonRoot: {{ .Values.securityContext.runAsNonRoot }}
    # You can ignore the rest of the block here!n }}
```
### B.2: Use a "Filtered" Helper (Advanced)
If you still want to use a helper but exclude one item, you can use the `omit` function. This tells Helm: "Take the whole `securityContext` block, but **omit** (leave out) this specific key."
...
- name: my-app
  securityContext:
    {{- .Values.securityContext | omit "readOnlyRootFilesystem" | toYaml | nindent 12 }}
```
---

The `{{- if }}` statement works just like a light switch for your Kubernetes files. It determines whether a piece of code (or an entire file) is **created** or **ignored**.

### How `if` works in Helm:
- **If **`**true**` **:** Helm **executes** the code inside and includes it in the final manifest.
- **If **`**false**` **:** Helm **deletes** the code inside. It acts as if that code never existed.
---

### The Best Example: The "Optional" Ingress
Imagine you are deploying an app. In **Production**, you want an Ingress (to access it via a URL), but in **Development**, you don't need it (you use `port-forward` instead).

#### 1. The `values.yaml` (The Switch)
YAML

```
# Set this to true to create the Ingress, false to skip it
ingress:
  enabled: true
```
#### 2. The `ingress.yaml` Template (The Logic)
YAML
...
{{- end }}
```
### What happens when Helm runs?
| **Value of enabled** | **Helm's Action** | **Result in Cluster** |
| ----- | ----- | ----- |
| `**true**`  | Helm looks inside the `if`, processes the YAML, and sends it to Kubernetes. | **Ingress is created.** |
| `**false**`  | Helm sees `false`, jumps straight to `{{- end }}`, and ignores everything in between. | <p>**Nothing happens.**</p><p> No Ingress is created.</p> |
---

### Combining `if` and `with`  
In professional charts, we use them together. The `if` is the **Gatekeeper**, and the `with` is the **Organizer**.

YAML

```
# 1. GATEKEEPER: Should we even build this?
{{- if .Values.ingress.enabled }}

apiVersion: networking.k8s.io/v1
kind: Ingress
spec:
  # 2. ORGANIZER: Let's make the variables easier to read
  {{- with .Values.ingress }}
